...
document.addEventListener('DOMContentLoaded', () => {
    ...
    addForm.addEventListener('submit', (event) => { 
        ...
        if (newFilm) {   // чтобы в список не попадала пустая строка, создаем условие (если newFilm булиновое true, то выполняем след.действия:)
   ...
   function createMoveList(films, parent) {
        ...
        films.forEach((film, i) => {   // при переборе массива movieDB.movies создаем колбек функцию с переменной  film и переменной i - номер фильма, который нужно добавить
            parent.innerHTML += `
   ...
   const tabs = document.querySelectorAll('.promo__menu-item'),
    ...
    function setDataDeadline(selector, deadlineStr) {
        ...
        if (promoDescr) {
            ...
            promoDescr.innerHTML = promoDescr.innerHTML.replace(
                /Акция закончится[^<]*/,
                `Акция закончится ${formattedDeadline} в 00:00`
            );
        }
    }
